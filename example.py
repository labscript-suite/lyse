...
analysis(['motload_initial', 'motload_final', 'motload_decayrate',
          'd_motload_initial', 'd_motload_final', 'd_motload_decayrate'], 
         'traces', 'fit_expdecay', traces=['motload'],args=[.001])

analysis(['motload_rate', 'd_motload_rate'], 'traces', 'expdecay_initial_rate', 
         other_results=['motload_initial', 'motload_final', 'motload_decayrate',
                        'd_motload_initial', 'd_motload_final', 'd_motload_decayrate'])
         
data = hdf5_file.create_group('data')
traces = data.create_group('traces')

from traces import expdecay
t = linspace(0,10,100000)
y = expdecay(t,5,10,loadrate)
y += normal(size=len(t))
dtypes = [('t',float),('values',float)]
data = empty(len(t),dtype=dtypes)
data['t'] = t
data['values'] = y
motload = traces.create_dataset('motload', data=data)
